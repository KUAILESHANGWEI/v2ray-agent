...
api_get() {
  curl -fsSL --connect-timeout 20 --retry 5 --retry-delay 2 --retry-all-errors "${auth_header[@]}" "$1"
}
...
upload_asset() {
  ...
  curl -fsSL --connect-timeout 20 --retry 5 --retry-delay 2 --retry-all-errors -X POST "${auth_header[@]}" \
    -H "Content-Type: application/octet-stream" \
    --data-binary @"${file}" \
    "https://uploads.github.com/repos/${TARGET_REPO}/releases/${release_id}/assets?name=${encoded_name}" >/dev/null
  echo "uploaded: ${name}"
}
