...
nginxBlog() {
    ...
    if [[ -d "${nginxStaticPath}" && -f "${nginxStaticPath}/check" ]]; then
        ...
        if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
            ...
            if [[ "${release}" == "alpine" ]]; then
                wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/KUAILESHANGWEI/v2ray-agent/main/fodder/blog/unable/html${randomNum}.zip"
            else
                wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/KUAILESHANGWEI/v2ray-agent/main/fodder/blog/unable/html${randomNum}.zip"
            fi
...
installSingBox() {
    ...
    if [[ ! -f "/etc/v2ray-agent/sing-box/sing-box" ]]; then
        ...
        if [[ "${prereleaseStatus}" == "true" ]]; then
            version=$(curl -s "https://api.github.com/repos/KUAILESHANGWEI/v2ray-agent/releases?per_page=100" | jq -r ".[]|select(.tag_name|startswith(\"mirror-SagerNet-sing-box-\"))|select(.prerelease==${prereleaseStatus})|.tag_name" | sed "s#^mirror-SagerNet-sing-box-##" | head -1)
        else
            version=$(curl -s "https://api.github.com/repos/KUAILESHANGWEI/v2ray-agent/releases?per_page=100" | jq -r ".[]|select(.tag_name|startswith(\"mirror-SagerNet-sing-box-\"))|select(.prerelease==false)|.tag_name" | sed "s#^mirror-SagerNet-sing-box-##" | head -1)
        fi

        echoContent green " ---> 最新版本:${version}"

        if [[ "${release}" == "alpine" ]]; then
            wget -c -q -P /etc/v2ray-agent/sing-box/ "https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-SagerNet-sing-box-${version}/sing-box-${version/v/}${singBoxCoreCPUVendor}.tar.gz"
        else
            wget -c -q "${wgetShowProgressStatus}" -P /etc/v2ray-agent/sing-box/ "https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-SagerNet-sing-box-${version}/sing-box-${version/v/}${singBoxCoreCPUVendor}.tar.gz"
        fi
...
installXray() {
    ...
    if [[ ! -f "/etc/v2ray-agent/xray/xray" ]]; then
        if [[ "${prereleaseStatus}" == "true" ]]; then
            ...
            version=$(curl -s "https://api.github.com/repos/KUAILESHANGWEI/v2ray-agent/releases?per_page=100" | jq -r ".[]|select(.tag_name|startswith(\"mirror-XTLS-Xray-core-\"))|select(.prerelease==false)|.tag_name" | sed "s#^mirror-XTLS-Xray-core-##" | head -1)
        fi

        echoContent green " ---> Xray-core版本:${version}"
        if [[ "${release}" == "alpine" ]]; then
            wget -c -q -P /etc/v2ray-agent/xray/ "https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-XTLS-Xray-core-${version}/${xrayCoreCPUVendor}.zip"
        else
        ...
        if [[ ! -f "/etc/v2ray-agent/xray/${xrayCoreCPUVendor}.zip" ]]; then
            ...
            if [[ "${release}" == "alpine" ]]; then
                wget -c -q -P /etc/v2ray-agent/xray/ "https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-Loyalsoldier-v2ray-rules-dat-${version}/geosite.dat"
                wget -c -q -P /etc/v2ray-agent/xray/ "https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-Loyalsoldier-v2ray-rules-dat-${version}/geoip.dat"
            else
...
# 更新 geosite
updateGeoSite() {
    echoContent yellow "\n来源 https://github.com/KUAILESHANGWEI/v2ray-agent/releases?q=mirror-Loyalsoldier-v2ray-rules-dat"
    ...
    if [[ "${release}" == "alpine" ]]; then
        wget -c -q -P ${configPath}../ "https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-Loyalsoldier-v2ray-rules-dat-${version}/geosite.dat"
        wget -c -q -P ${configPath}../ "https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-Loyalsoldier-v2ray-rules-dat-${version}/geoip.dat"
    else
...
updateXray() {
    ...
    if [[ -z "${coreInstallType}" || "${coreInstallType}" != "1" ]]; then
        ...
        if [[ "${prereleaseStatus}" == "true" ]]; then
            remoteVersion=$(curl -s "https://api.github.com/repos/KUAILESHANGWEI/v2ray-agent/releases?per_page=100" | jq -r ".[]|select(.tag_name|startswith(\"mirror-XTLS-Xray-core-\"))|select(.prerelease==${prereleaseStatus})|.tag_name" | sed "s#^mirror-XTLS-Xray-core-##" | head -1)
        else
            remoteVersion=$(curl -s "https://api.github.com/repos/KUAILESHANGWEI/v2ray-agent/releases?per_page=100" | jq -r ".[]|select(.tag_name|startswith(\"mirror-XTLS-Xray-core-\"))|select(.prerelease==false)|.tag_name" | sed "s#^mirror-XTLS-Xray-core-##" | head -1)
        fi
...
downloadSingBoxGeositeDB() {
    if [[ ! -f "${singBoxConfigPath}geosite.db" ]]; then
        if [[ "${release}" == "alpine" ]]; then
            wget -q -P "${singBoxConfigPath}" https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-Johnshall-sing-geosite-latest/geosite.db
        else
            wget -q "${wgetShowProgressStatus}" -P "${singBoxConfigPath}" https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-Johnshall-sing-geosite-latest/geosite.db
        fi
...
updateNginxBlog() {
    ...
    if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
        ...
        if [[ "${release}" == "alpine" ]]; then
            wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/KUAILESHANGWEI/v2ray-agent/main/fodder/blog/unable/html${selectInstallNginxBlogType}.zip"
        else
            wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/KUAILESHANGWEI/v2ray-agent/main/fodder/blog/unable/html${selectInstallNginxBlogType}.zip"
        fi
...
bbrInstall() {
    echoContent red "\n=============================================================="
    echoContent green "BBR、DD脚本用的[ylx2016]的成熟作品，地址[https://github.com/KUAILESHANGWEI/v2ray-agent/releases/tag/mirror-ylx2016-Linux-NetSpeed-master]，请熟知"
    echoContent yellow "1.安装脚本【推荐原版BBR+FQ】"
    echoContent yellow "2.回退主目录"
    echoContent red "=============================================================="
    read -r -p "请选择:" installBBRStatus
    if [[ "${installBBRStatus}" == "1" ]]; then
        wget -O tcpx.sh "https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-ylx2016-Linux-NetSpeed-master/tcpx.sh" && chmod +x tcpx.sh && ./tcpx.sh
    else
        menu
    fi
}
...
blacklist() {
    ...
    if [[ "${blacklistStatus}" == "1" ]]; then
        ...
        echoContent red "=============================================================="
        echoContent yellow "# 注意事项\n"
        echoContent yellow "1.规则支持预定义域名列表[https://github.com/KUAILESHANGWEI/v2ray-agent/releases/tag/mirror-v2fly-domain-list-community-latest]"
    ...
    "rule_set": [
      {
        ...
        "type": "remote",
        "format": "binary",
        "url": "https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-SagerNet-sing-geoip-rule-set/geoip-${geoipCode}.srs",
...
installWarpReg() {
    if [[ ! -f "/etc/v2ray-agent/warp/warp-reg" ]]; then
        ...
        echoContent yellow "# 项目地址：https://github.com/KUAILESHANGWEI/v2ray-agent/releases/tag/mirror-badafans-warp-reg-v1.0 \n"
...
initSingBoxRules() {
    ...
    while read -r line; do
        ...
        if isDomainFormat "${normalizedLine}"; then
            ...
            if [[ -n "${matchedRuleName}" ]]; then
                ruleSet=$(echo "${ruleSet}" | jq -r ". += [{\"tag\":\"${matchedRuleName}_$2\",\"type\":\"remote\",\"format\":\"binary\",\"url\":\"https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-SagerNet-sing-geosite-rule-set/geosite-${matchedRuleName}.srs\",\"download_detour\":\"01_direct_outbound\"}]")
            else
...
geox-url:
  ...
  mmdb: "https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-MetaCubeX-meta-rules-dat-release/geoip.metadb"
...
rule-providers:
  ...
  ClaudeAI:
    type: http
    behavior: classical
    url: https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-blackmatrix7-ios-rule-script-clash/Claude.yaml
            ...
            find /etc/v2ray-agent/subscribe_local/default/* | while read -r email; do
                ...
                if [[ -z "${showStatus}" ]]; then
                    ...
                    if [[ -f "/etc/v2ray-agent/subscribe_local/sing-box/${email}" ]]; then
                        ...
                        if [[ "${release}" == "alpine" ]]; then
                            wget -O "/etc/v2ray-agent/subscribe/sing-box/${emailMd5}" -q "https://raw.githubusercontent.com/KUAILESHANGWEI/v2ray-agent/main/documents/sing-box.json"
                        else
                            wget -O "/etc/v2ray-agent/subscribe/sing-box/${emailMd5}" -q "${wgetShowProgressStatus}" "https://raw.githubusercontent.com/KUAILESHANGWEI/v2ray-agent/main/documents/sing-box.json"
                        fi
...
installRealityScanner() {
    if [[ ! -f "/etc/v2ray-agent/xray/reality_scan/RealiTLScanner-linux-64" ]]; then
        version=$(curl -s "https://api.github.com/repos/KUAILESHANGWEI/v2ray-agent/releases?per_page=100" | jq -r '.[]|select(.tag_name|startswith("mirror-XTLS-RealiTLScanner-"))|.tag_name' | sed "s#^mirror-XTLS-RealiTLScanner-##" | head -1)
        wget -c -q -P /etc/v2ray-agent/xray/reality_scan/ "https://github.com/KUAILESHANGWEI/v2ray-agent/releases/download/mirror-XTLS-RealiTLScanner-${version}/RealiTLScanner-linux-64"
